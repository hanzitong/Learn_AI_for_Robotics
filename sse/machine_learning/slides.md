...
# 正則化
<!-- video4 15:18 -->
コスト関数に正則化項を追加して特徴量をうまく選択させましょう。という話。
- 正則化項をコスト関数に入れて過学習しづらくする。
- どの特徴量を選ぶかは難しい問題。
  - 何が起きているかの現象がイメージできれば参考にできる。
  - 領域に関する知識が少ないと、どれを入れていいか全然わからなかったりする。
  - 現場の方などから有識者に説明してもらいながら、特徴量を選択する。信頼してもらえないと話を聞けなかったりするらしい。
- 正則化項のいいところ：入力した特徴量の中から、勝手に重要な数個を選んでくれる。

> 制約条件っぽく作用する項を入れて解を絞ってる。使うときに調べましょう。
...
# 非線形問題を解くためのカーネル法
<!-- video4 44:00 -->
- データをうまく変換した先で分離境界を見つけるというのがカーネルのアイディア。
- データが複雑なほどどういう変換をかければ良いのかを見つけるのは職人技。
- 誰でもうまく扱う仕組みがカーネル法。
  - 線形カーネル
  - 多項式カーネル
  - RBFカーネル(ガウスカーネル)
